...
static void zombie_update_ai(Entity *entity, float delta_time) {
    ...
    if (entity->data.zombie.is_walking) {
        ...
        if (entity->data.zombie.state_timer <= 0.0f) {
            ...
            entity->data.zombie.state_timer = IDLE_TIME_MIN +
                ((float)rand() / (float)RAND_MAX) * (IDLE_TIME_MAX - IDLE_TIME_MIN);
        }
    } else {
        if (entity->data.zombie.state_timer <= 0.0f) {
            float angle = ((float)rand() / (float)RAND_MAX) * 2.0f * (float)M_PI;
            entity->data.zombie.walk_direction = vec3(cosf(angle), 0.0f, sinf(angle));
            entity->data.zombie.yaw = angle - (float)M_PI / 2.0f;
            entity->data.zombie.is_walking = true;
            entity->data.zombie.state_timer = WALK_TIME_MIN +
                ((float)rand() / (float)RAND_MAX) * (WALK_TIME_MAX - WALK_TIME_MIN);
