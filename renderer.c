...
struct Renderer {
    ...
    /* Texture data (previously Texture textures[BLOCK_TYPE_COUNT]) */
    VkImage texture_images[BLOCK_TYPE_COUNT];
    VkDeviceMemory texture_memories[BLOCK_TYPE_COUNT];
    VkImageView texture_views[BLOCK_TYPE_COUNT];
    VkSampler texture_samplers[BLOCK_TYPE_COUNT];
    uint32_t texture_widths[BLOCK_TYPE_COUNT];
    uint32_t texture_heights[BLOCK_TYPE_COUNT];

    /* Black texture data (previously Texture black_texture) */
    ...
    /* Swapchain resources (previously SwapchainResources struct) */
...
static void texture_create_from_pixels(VkDevice device,
    ...
    transition_image_layout(device, command_pool, graphics_queue,
                            *out_image, VK_FORMAT_R8G8B8A8_SRGB,
    ...
    create_image_view(device, *out_image, VK_FORMAT_R8G8B8A8_SRGB,
                      VK_IMAGE_ASPECT_COLOR_BIT, out_view);
    ...
    VK_CHECK(vkCreateSampler(device, &sampler_info, NULL, out_sampler));
...
void texture_create_from_file(VkDevice device,
    ...
    texture_create_from_pixels(device, physical_device, command_pool, graphics_queue,
                               ...
                               out_image, out_memory, out_view, out_sampler, out_width, out_height);
...
void swapchain_resources_reset(Renderer *renderer) {
    ...
    renderer->pipeline_solid = VK_NULL_HANDLE;
    renderer->pipeline_wireframe = VK_NULL_HANDLE;
    renderer->pipeline_crosshair = VK_NULL_HANDLE;
    renderer->pipeline_overlay = VK_NULL_HANDLE;
    renderer->descriptor_pool = VK_NULL_HANDLE;
    renderer->descriptor_sets_normal = NULL;
    renderer->descriptor_sets_highlight = NULL;
    renderer->command_buffers = NULL;
    renderer->depth_image = VK_NULL_HANDLE;
    renderer->depth_memory = VK_NULL_HANDLE;
    renderer->depth_view = VK_NULL_HANDLE;
...
void swapchain_destroy(Renderer *renderer) {
    ...
    if (renderer->command_buffers) {
        vkFreeCommandBuffers(device, command_pool, renderer->swapchain_image_count, renderer->command_buffers);
        free(renderer->command_buffers);
        renderer->command_buffers = NULL;
    }

    VK_DESTROY(device, DescriptorPool, renderer->descriptor_pool);
    ...
    VK_DESTROY(device, Pipeline, renderer->pipeline_crosshair);
    VK_DESTROY(device, Pipeline, renderer->pipeline_overlay);
    VK_DESTROY(device, Pipeline, renderer->pipeline_wireframe);
    VK_DESTROY(device, Pipeline, renderer->pipeline_solid);

    VK_DESTROY(device, RenderPass, renderer->render_pass);

    VK_DESTROY(device, ImageView, renderer->depth_view);
    VK_DESTROY(device, Image, renderer->depth_image);
    if (renderer->depth_memory != VK_NULL_HANDLE) {
    ...
    free(renderer->descriptor_sets_normal);
    free(renderer->descriptor_sets_highlight);
    renderer->descriptor_sets_normal = NULL;
    renderer->descriptor_sets_highlight = NULL;
...
void swapchain_create(Renderer *renderer,
    ...
    create_image_view(device, renderer->depth_image, depth_format, VK_IMAGE_ASPECT_DEPTH_BIT, &renderer->depth_view);
    ...
    VK_CHECK(vkCreateRenderPass(device, &render_pass_info, NULL, &renderer->render_pass));
    ...
    VK_CHECK(vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipeline_info, NULL, &renderer->pipeline_solid));
    ...
    VK_CHECK(vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipeline_info, NULL, &renderer->pipeline_wireframe));

    pipeline_info.pRasterizationState = &raster_wire;
    pipeline_info.pDepthStencilState = &depth_cross;
    VK_CHECK(vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipeline_info, NULL, &renderer->pipeline_crosshair));

    pipeline_info.pInputAssemblyState = &input_assembly_tri;
    pipeline_info.pRasterizationState = &raster_solid;
    pipeline_info.pDepthStencilState = &depth_cross;
    VK_CHECK(vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipeline_info, NULL, &renderer->pipeline_overlay));

    renderer->swapchain_framebuffers = malloc(sizeof(VkFramebuffer) * image_count);
    for (uint32_t i = 0; i < image_count; ++i) {
        VkImageView attachments_views[] = {renderer->swapchain_image_views[i], renderer->depth_view};
    ...
    VK_CHECK(vkCreateDescriptorPool(device, &pool_info, NULL, &renderer->descriptor_pool));
    ...
    renderer->descriptor_sets_normal = malloc(sizeof(VkDescriptorSet) * image_count);
    renderer->descriptor_sets_highlight = malloc(sizeof(VkDescriptorSet) * image_count);

    VK_CHECK(vkAllocateDescriptorSets(device, &alloc_info, renderer->descriptor_sets_normal));
    VK_CHECK(vkAllocateDescriptorSets(device, &alloc_info, renderer->descriptor_sets_highlight));
    ...
    for (uint32_t i = 0; i < image_count; ++i) {
        ...
        for (uint32_t tex = 0; tex < BLOCK_TYPE_COUNT; ++tex) {
            ...
            normal_images[tex].imageView = renderer->texture_views[tex];
            normal_images[tex].sampler = renderer->texture_samplers[tex];
        ...
        write.dstSet = renderer->descriptor_sets_normal[i];
        write.pImageInfo = normal_images;
        vkUpdateDescriptorSets(device, 1, &write, 0, NULL);

        write.dstSet = renderer->descriptor_sets_highlight[i];
        write.pImageInfo = highlight_images;
        vkUpdateDescriptorSets(device, 1, &write, 0, NULL);
    }

    renderer->command_buffers = malloc(sizeof(VkCommandBuffer) * image_count);
    ...
    VK_CHECK(vkAllocateCommandBuffers(device, &command_alloc, renderer->command_buffers));
}
...
Renderer *renderer_create(void *display,
    ...
    for (uint32_t i = 0; i < BLOCK_TYPE_COUNT; ++i) {
        texture_create_from_file(renderer->device, renderer->physical_device,
                                 ...
                                 texture_files[i],
                                 &renderer->texture_images[i],
                                 &renderer->texture_memories[i],
                                 &renderer->texture_views[i],
                                 &renderer->texture_samplers[i],
                                 &renderer->texture_widths[i],
                                 &renderer->texture_heights[i]);
    }
...
bool renderer_draw_frame(Renderer *renderer,
    ...
    VK_CHECK(vkResetCommandBuffer(renderer->command_buffers[image_index], 0));

    VkCommandBufferBeginInfo begin_info = {0};
    begin_info.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
    VK_CHECK(vkBeginCommandBuffer(renderer->command_buffers[image_index], &begin_info));
    ...
    rp_begin.renderPass = renderer->render_pass;
    ...
    vkCmdBeginRenderPass(renderer->command_buffers[image_index], &rp_begin, VK_SUBPASS_CONTENTS_INLINE);

    VkDeviceSize offsets[2] = {0, 0};
    VkBuffer vbs[2];

    vkCmdBindPipeline(renderer->command_buffers[image_index], VK_PIPELINE_BIND_POINT_GRAPHICS, renderer->pipeline_solid);
    vkCmdPushConstants(renderer->command_buffers[image_index], renderer->pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(pc), &pc);

    vbs[0] = renderer->block_vertex_buffer;
    vbs[1] = renderer->instance_buffer;
    vkCmdBindVertexBuffers(renderer->command_buffers[image_index], 0, 2, vbs, offsets);
    vkCmdBindIndexBuffer(renderer->command_buffers[image_index], renderer->block_index_buffer, 0, VK_INDEX_TYPE_UINT16);

    vkCmdBindDescriptorSets(renderer->command_buffers[image_index],
                            VK_PIPELINE_BIND_POINT_GRAPHICS,
                            renderer->pipeline_layout,
                            0,
                            1,
                            &renderer->descriptor_sets_normal[image_index],
                            0,
                            NULL);

    if (render_blocks > 0) {
        vkCmdDrawIndexed(renderer->command_buffers[image_index],
    ...
    if (highlight) {
        vkCmdBindPipeline(renderer->command_buffers[image_index], VK_PIPELINE_BIND_POINT_GRAPHICS, renderer->pipeline_wireframe);
        vkCmdPushConstants(renderer->command_buffers[image_index], renderer->pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(pc), &pc);

        vbs[0] = renderer->edge_vertex_buffer;
        vbs[1] = renderer->instance_buffer;
        vkCmdBindVertexBuffers(renderer->command_buffers[image_index], 0, 2, vbs, offsets);
        vkCmdBindIndexBuffer(renderer->command_buffers[image_index], renderer->edge_index_buffer, 0, VK_INDEX_TYPE_UINT16);
        ...
        vkCmdBindDescriptorSets(renderer->command_buffers[image_index],
                                ...
                                0,
                                1,
                                &renderer->descriptor_sets_highlight[image_index],
                                0,
                                NULL);
    ...
    if (!player->inventory_open) {
        vkCmdBindPipeline(renderer->command_buffers[image_index], VK_PIPELINE_BIND_POINT_GRAPHICS, renderer->pipeline_crosshair);
        vkCmdPushConstants(renderer->command_buffers[image_index], renderer->pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(pc_overlay), &pc_overlay);
        ...
        vkCmdDraw(renderer->command_buffers[image_index], 4, 1, 0, crosshair_instance_index);
    }

    if (player->inventory_open && renderer->inventory_vertex_count > 0) {
        if (renderer->inventory_bg_vertex_count > 0) {
            vkCmdBindPipeline(renderer->command_buffers[image_index], VK_PIPELINE_BIND_POINT_GRAPHICS, renderer->pipeline_overlay);
            vkCmdPushConstants(renderer->command_buffers[image_index], renderer->pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(pc_overlay), &pc_overlay);
            ...
            vkCmdDraw(renderer->command_buffers[image_index], renderer->inventory_bg_vertex_count, 1, 0, inventory_bg_instance_index);
        }
        ...
        vkCmdDraw(renderer->command_buffers[image_index], renderer->inventory_vertex_count, 1, 0, inventory_instance_index);
    }
    ...
    if (player->inventory_open && renderer->inventory_selection_vertex_count > 0) {
        ...
        vkCmdDraw(renderer->command_buffers[image_index], renderer->inventory_selection_vertex_count, 1, 0, inventory_selection_instance_index);
    }
    ...
    if (player->inventory_open && inventory_icon_count > 0) {
        ...
        vkCmdDraw(renderer->command_buffers[image_index], renderer->inventory_icon_vertex_count, inventory_icon_count, 0, inventory_icons_start);
    }
    ...
    if (player->inventory_open && renderer->inventory_count_vertex_count > 0) {
        ...
        vkCmdDraw(renderer->command_buffers[image_index], renderer->inventory_count_vertex_count, 1, 0, inventory_instance_index);
    }

    vkCmdEndRenderPass(renderer->command_buffers[image_index]);
    VK_CHECK(vkEndCommandBuffer(renderer->command_buffers[image_index]));
    ...
    submit.pCommandBuffers = &renderer->command_buffers[image_index];
