...
static uint32_t fill_instance_buffer(Renderer *r, World *world, const Player *player, float aspect,
    ...
    int world_block_count = world_total_render_blocks(world);
    uint32_t entity_block_count = world_entity_block_count(world);
    uint32_t block_count = (uint32_t)world_block_count + entity_block_count;
    ...
    for (int i = 0; i < world->chunk_count; i++) {
        ...
        for (int j = 0; j < chunk->block_count; j++) {
            ...
            instances[idx++] = (InstanceData){b.pos.x, b.pos.y, b.pos.z, b.type, 1.0f, 1.0f, 1.0f, 0.0f, 0.0f};
        }
    }

    if (entity_block_count > 0) {
        RenderBlock *blocks = malloc(sizeof(RenderBlock) * entity_block_count);
        if (!blocks) {
    ...
    *out_icons_start = idx;
    ...
    instances[*out_highlight_idx] = (InstanceData){
        ...
        highlight ? (float)highlight_cell.z : 0, HIGHLIGHT_TEXTURE_INDEX, 1.0f, 1.0f, 1.0f, 0.0f, 0.0f
    };
    instances[*out_crosshair_idx] = (InstanceData){0, 0, 0, CROSSHAIR_TEXTURE_INDEX, 1.0f, 1.0f, 1.0f, 0.0f, 0.0f};
    instances[*out_inventory_idx] = (InstanceData){0, 0, 0, HIGHLIGHT_TEXTURE_INDEX, 1.0f, 1.0f, 1.0f, 0.0f, 0.0f};
    instances[*out_selection_idx] = (InstanceData){0, 0, 0, INVENTORY_SELECTION_TEXTURE_INDEX, 1.0f, 1.0f, 1.0f, 0.0f, 0.0f};
    instances[*out_bg_idx] = (InstanceData){0, 0, 0, INVENTORY_BG_TEXTURE_INDEX, 1.0f, 1.0f, 1.0f, 0.0f, 0.0f};
    instances[*out_health_bg_idx] = (InstanceData){0, 0, 0, HEALTH_BAR_INDEX, 1.0f, 1.0f, 1.0f, 0.0f, 0.0f};
    instances[*out_health_border_idx] = (InstanceData){0, 0, 0, HIGHLIGHT_TEXTURE_INDEX, 1.0f, 1.0f, 1.0f, 0.0f, 0.0f};
...
void renderer_draw_frame(Renderer *r, World *world, const Player *player, Camera *camera,
    ...
    int block_count = world_total_render_blocks(world) + (int)world_entity_block_count(world);
